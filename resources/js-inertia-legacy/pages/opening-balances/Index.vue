...
const { t } = useI18n();
const { data, meta, loading, error, refresh } = useApiResource<Record<string, unknown>[]>('/api/v1/opening-balances');
const rows = computed(() => (Array.isArray(data.value) ? data.value : []));
const columns = computed(() => [
    { key: 'as_of', label: t('app.date') },
...
<template>
    <div>
        <PageHeader :title="t('openingBalances.title')">
            <template #actions>
