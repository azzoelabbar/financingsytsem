...
const { t } = useI18n();
const { data, meta, loading, error, refresh } = useApiResource<Record<string, unknown>[]>('/api/v1/expenses');
const rows = computed(() => (Array.isArray(data.value) ? data.value : []));
const columns = computed(() => [
    { key: 'number', label: t('app.number') },
    { key: 'claim_date', label: t('app.date') },
    ...
    <div>
        <PageHeader :title="t('expenses.title')">
            <template #actions>
                ...
                <Button size="sm" as-child><Link href="/expenses/create">{{ t('app.create') }}</Link></Button>
            </template>
        </PageHeader>
