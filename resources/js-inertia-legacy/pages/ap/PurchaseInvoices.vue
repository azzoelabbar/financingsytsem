...
const { t } = useI18n();
const { data, meta, loading, error, refresh } = useApiResource<Record<string, unknown>[]>('/api/v1/ap/purchase-invoices');
const rows = computed(() => (Array.isArray(data.value) ? data.value : []));
const columns = computed(() => [
    { key: 'number', label: t('app.number') },
    { key: 'invoice_date', label: t('app.date') },
...
<template>
    <div>
        <PageHeader :title="t('ap.invoices')">
            <template #actions>
                <Button variant="outline" size="sm" @click="refresh">{{ t('app.refresh') }}</Button>
                <Button size="sm" as-child><Link href="/ap/purchase-invoices/create">{{ t('app.create') }}</Link></Button>
            </template>
        </PageHeader>
