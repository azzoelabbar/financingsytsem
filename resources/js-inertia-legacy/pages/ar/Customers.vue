...
const { t } = useI18n();
const { data, meta, loading, error, refresh } = useApiResource<Record<string, unknown>[]>('/api/v1/ar/customers');
const rows = computed(() => (Array.isArray(data.value) ? data.value : []));
const columns = computed(() => [
    { key: 'code', label: t('app.code') },
    { key: 'name_ar', label: t('app.name') },
...
<template>
    <div>
        <PageHeader :title="t('ar.customers')">
            <template #actions>
                <Button variant="outline" size="sm" @click="refresh">{{ t('app.refresh') }}</Button>
                <Button size="sm" as-child><Link href="/ar/customers/create">{{ t('app.create') }}</Link></Button>
            </template>
        </PageHeader>
