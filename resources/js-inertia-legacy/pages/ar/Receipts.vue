...
const { t } = useI18n();
const { data, meta, loading, error, refresh } = useApiResource<Record<string, unknown>[]>('/api/v1/ar/receipts');
const rows = computed(() => (Array.isArray(data.value) ? data.value : []));
const columns = computed(() => [
    { key: 'number', label: t('app.number') },
    { key: 'receipt_date', label: t('app.date') },
...
<template>
    <div>
        <PageHeader :title="t('ar.receipts')">
            <template #actions>
                <Button variant="outline" size="sm" @click="refresh">{{ t('app.refresh') }}</Button>
                <Button size="sm" as-child><Link href="/ar/receipts/create">{{ t('app.create') }}</Link></Button>
            </template>
        </PageHeader>
