...
const { t } = useI18n();
const { data, meta, loading, error, refresh } = useApiResource<Record<string, unknown>[]>('/api/v1/ar/sales-invoices');
    ...
    <div>
        <PageHeader :title="t('ar.invoices')">
            <template #actions>
                ...
                <Button size="sm" as-child><Link href="/ar/sales-invoices/create">{{ t('app.create') }}</Link></Button>
            </template>
        </PageHeader>
