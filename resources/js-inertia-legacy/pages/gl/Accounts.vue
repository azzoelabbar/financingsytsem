...
const { t } = useI18n();
const { data, meta, loading, error, refresh } = useApiResource<Record<string, unknown>[]>('/api/v1/gl/accounts');
const rows = computed(() => (Array.isArray(data.value) ? data.value : []));
const columns = computed(() => [
    { key: 'code', label: t('app.code') },
    { key: 'name_ar', label: t('app.name') },
    { key: 'account_type', label: t('app.status') }
]);
</script>

<template>
    <div>
        <PageHeader :title="t('gl.accounts')">
            <template #actions>
