...
const { t } = useI18n();
const { data, meta, loading, error, refresh } = useApiResource<Record<string, unknown>[]>('/api/v1/gl/journals');
const rows = computed(() => (Array.isArray(data.value) ? data.value : []));
const columns = computed(() => [
    { key: 'number', label: t('app.number') },
    { key: 'journal_date', label: t('app.date') },
    { key: 'status', label: t('app.status') },
    { key: 'source', label: t('app.reference') }
]);
</script>

<template>
    <div>
        <PageHeader :title="t('gl.journals')">
            <template #actions>
                <Button variant="outline" size="sm" @click="refresh">{{ t('app.refresh') }}</Button>
                <Button size="sm" as-child><Link href="/gl/journals/create">{{ t('app.create') }}</Link></Button>
            </template>
        </PageHeader>
