...
const { t } = useI18n();
const { data, meta, loading, error, refresh } = useApiResource<Record<string, unknown>[]>('/api/v1/gl/periods');
const rows = computed(() => (Array.isArray(data.value) ? data.value : []));
const columns = computed(() => [
    { key: 'period_no', label: t('app.number') },
    { key: 'start_date', label: t('app.from') },
    { key: 'end_date', label: t('app.to') },
