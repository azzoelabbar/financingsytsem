<div>
    ...
    @if ($status === 'draft')
        <form wire:submit="addLine" class="mb-5"><x-ui.card :title="__('erp.opening.add_line')"><div class="grid items-end gap-4 md:grid-cols-4"><x-ui.field :label="__('erp.account')" for="opening-account" required :error="$errors->first('accountCode')"><select id="opening-account" wire:model="accountCode" class="erp-control"><option value="">{{ __('erp.select') }}</option>@foreach($accounts as $account)<option value="{{ $account->code }}">{{ $account->code }} — {{ app()->getLocale()==='ar'?$account->name_ar:($account->name_en??$account->name_ar) }}</option>@endforeach</select></x-ui.field><x-ui.field :label="__('erp.opening.side')" for="opening-side" required><select id="opening-side" wire:model="side" class="erp-control"><option value="debit">{{ __('erp.debit') }}</option><option value="credit">{{ __('erp.credit') }}</option></select></x-ui.field><x-ui.field :label="__('erp.amount')" for="opening-amount" required :error="$errors->first('amount')"><input id="opening-amount" type="number" min="0.000001" step="0.000001" wire:model="amount" class="erp-control text-end tabular-nums" dir="ltr"/></x-ui.field><x-ui.button type="submit">{{ __('erp.opening.add_line') }}</x-ui.button></div></x-ui.card></form>
    @endif
