@php
    ...
    $supName = app()->getLocale() === 'ar' ? ($note->supplier?->name_ar ?? $note->supplier?->legal_name) : ($note->supplier?->trading_name ?? $note->supplier?->legal_name ?? $note->supplier?->name_ar);
@endphp
<x-doc.workspace
    ...
    :status="$note->status"
    :backRoute="$kind === 'credit' ? route('ap.credit-notes') : route('ap.debit-notes')"
    ...
    :breadcrumbs="[
        ['label' => __('erp.nav.ap')],
        ['label' => $docTitle, 'href' => $kind === 'credit' ? route('ap.credit-notes') : route('ap.debit-notes')],
...
>
    ...
    <x-ui.card :title="__('erp.form.parties')">
        <dl class="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-3">
            <div><dt class="text-xs text-muted-foreground">{{ __('erp.purchase_invoice.supplier') }}</dt><dd class="mt-0.5 text-sm font-medium">{{ $supName }}</dd></div>
            ...
            @if ($note->originalInvoice)
                <div><dt class="text-xs text-muted-foreground">{{ __('erp.document.original_invoice') }}</dt><dd class="mt-0.5 text-sm font-medium">{{ $note->originalInvoice->number }}</dd></div>
            @endif
        </dl>
    </x-ui.card>
    ...
    @if($kind==='credit' && $note->status->isPosted())
        <x-ui.card :title="__('erp.receipt.allocations')" :description="__('erp.note.ap_allocations_hint')" flush>
            ...
            @endif
            @if($note->allocations->isNotEmpty())<x-ui.table flush><thead><tr><th>{{ __('erp.payment.invoice') }}</th><th>{{ __('erp.date') }}</th><th class="!text-end">{{ __('erp.receipt.alloc_amount') }}</th></tr></thead><tbody>@foreach($note->allocations as $allocation)<tr><td><a href="{{ route('ap.invoices.show',$allocation->purchase_invoice_id) }}" wire:navigate class="text-[var(--brand-600)]">{{ $allocation->invoice?->number }}</a></td><td dir="ltr">{{ $allocation->allocation_date?->format('Y-m-d') }}</td><td class="text-end"><x-ui.money :amount="$allocation->amount" :currency="$note->currency" /></td></tr>@endforeach</tbody></x-ui.table>@else<x-ui.empty-state variant="panel" :message="__('erp.note.no_allocations')" />@endif
        </x-ui.card>
    @endif
</x-doc.workspace>
