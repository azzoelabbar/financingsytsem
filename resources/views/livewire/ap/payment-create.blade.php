<div>
    ...
    <form wire:submit="save" class="mx-auto max-w-5xl overflow-hidden rounded-lg border border-border bg-card">
        <x-ui.form-section :title="__('erp.form.parties')" :description="__('erp.payment.parties_hint')">
            <x-ui.field :label="__('erp.purchase_invoice.supplier')" for="payment-supplier" required :error="$errors->first('supplier_id')"><select id="payment-supplier" wire:model.live="supplier_id" class="erp-control" required><option value="">{{ __('erp.purchase_invoice.select_supplier') }}</option>@foreach($suppliers as $supplier)<option value="{{ $supplier->id }}">{{ $supplier->code }} — {{ $supplier->legal_name }}</option>@endforeach</select></x-ui.field>
        ...
        <x-ui.form-section :title="__('erp.payment.details')" :description="__('erp.payment.details_hint')">
            ...
            <x-ui.field :label="__('erp.payment.method')" for="payment-method" required><select id="payment-method" wire:model="method" class="erp-control"><option value="bank">{{ __('erp.receipt.method_bank') }}</option><option value="cash">{{ __('erp.receipt.method_cash') }}</option></select></x-ui.field>
            <x-ui.field :label="__('erp.receipt.cash_bank_account')" for="payment-account" required :error="$errors->first('cash_bank_account')"><select id="payment-account" wire:model="cash_bank_account" class="erp-control">@foreach($accounts as $account)<option value="{{ $account->code }}">{{ $account->code }} — {{ app()->getLocale()==='ar' ? $account->name_ar : ($account->name_en ?? $account->name_ar) }}</option>@endforeach</select></x-ui.field>
