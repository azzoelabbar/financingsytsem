...
<div>
    ...
    <form wire:submit="save" class="mx-auto max-w-5xl overflow-hidden rounded-lg border border-border bg-card">
        <x-ui.form-section :title="__('erp.form.parties')" :description="__('erp.note.ap_parties_hint')">
            <x-ui.field :label="__('erp.purchase_invoice.supplier')" for="ap-note-supplier" required :error="$errors->first('supplier_id')"><select id="ap-note-supplier" wire:model.live="supplier_id" class="erp-control" required><option value="">{{ __('erp.purchase_invoice.select_supplier') }}</option>@foreach($suppliers as $supplier)<option value="{{ $supplier->id }}">{{ $supplier->code }} — {{ $supplier->legal_name }}</option>@endforeach</select></x-ui.field>
            ...
            <x-ui.field :label="__('erp.document.original_invoice')" for="ap-note-invoice" :error="$errors->first('purchase_invoice_id')"><select id="ap-note-invoice" wire:model="purchase_invoice_id" class="erp-control"><option value="">{{ __('erp.note.no_related_invoice') }}</option>@foreach($invoices as $invoice)<option value="{{ $invoice->id }}">{{ $invoice->number }} — {{ number_format((float)$invoice->openBalance(),2) }} {{ $invoice->currency }}</option>@endforeach</select></x-ui.field>
