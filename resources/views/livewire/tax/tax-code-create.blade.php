<div>
    ...
    <form wire:submit="save" class="mx-auto max-w-4xl overflow-hidden rounded-lg border border-border bg-card">
        <x-ui.form-section :title="__('erp.tax.code_details')">
            ...
            <x-ui.searchable-select id="tax-kind" wire:model="kind" :label="__('erp.tax.kind')" required>@foreach(['output_vat','input_vat','wht','cit','deferred'] as $kind)<option value="{{ $kind }}">{{ __('erp.tax.kinds.'.$kind) }}</option>@endforeach</x-ui.searchable-select>
            <x-ui.searchable-select :label="__('erp.tax.gl_account')" required :error="$errors->first('gl_account_code')" id="tax-account" wire:model="gl_account_code"><option value="">{{ __('erp.select') }}</option>@foreach($accounts as $account)<option value="{{ $account->code }}">{{ $account->code }} — {{ app()->getLocale()==='ar'?$account->name_ar:($account->name_en??$account->name_ar) }}</option>@endforeach</x-ui.searchable-select>
