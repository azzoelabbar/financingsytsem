...
<div>
    ...
    <form wire:submit="save" class="mx-auto max-w-5xl overflow-hidden rounded-lg border border-border bg-card">
        <x-ui.form-section :title="__('erp.form.parties')" :description="__('erp.note.parties_hint')">
            <x-ui.searchable-select :label="__('erp.sales_invoice.customer')" required :error="$errors->first('customer_id')" id="note-customer" wire:model.live="customer_id" required>
            ...
            <x-ui.searchable-select :label="__('erp.document.original_invoice')" :error="$errors->first('sales_invoice_id')" :hint="__('erp.note.invoice_hint')" id="note-invoice" wire:model="sales_invoice_id">
