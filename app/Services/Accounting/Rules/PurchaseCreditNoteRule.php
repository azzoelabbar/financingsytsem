...
/**
 * Purchase credit note → Dr AP · Cr Expense/Inventory · Cr Input Tax.
 * Payload: ap_account, book_basis, lines: [{ expense_account, tax_account, net, tax, dimensions }].
 */
final class PurchaseCreditNoteRule implements AccountingRule
{
    public function type(): string
    {
        return 'purchase.credit_note';
    }
    ...
    public function build(SourceDocument $document): JournalDraft
    {
        ...
        if ($apAccount === '') {
            throw new PostingException('Purchase credit note payload must include the AP control account resolved from the chart.');
        }
        ...
        foreach ($lines as $line) {
            ...
            if ($expenseAccount === '') {
                throw new PostingException('Each credit-note line must carry an expense or inventory account.');
            }
            ...
            $creditMovements[] = LedgerMovement::credit($expenseAccount, $net, $lineDimensions, 'Expense reversal');
            if (Decimal::isPositive($tax)) {
                if ($taxAccount === '') {
                    throw new PostingException('A credit-note line with tax must carry a tax account code.');
                }
                $creditMovements[] = LedgerMovement::credit($taxAccount, $tax, $lineDimensions, 'Input VAT reversal');
            }
            $gross = Decimal::add($gross, Decimal::add($net, $tax));
        }

        $movements = array_merge(
            [LedgerMovement::debit($apAccount, $gross, $docDimensions, 'Reduce payable')],
            $creditMovements,
        );
        ...
        return new JournalDraft(
            ...
            description: 'Purchase credit note '.($document->reference() ?? ''),
            movements: $movements,
