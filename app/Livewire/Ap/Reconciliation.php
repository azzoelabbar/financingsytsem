...
class Reconciliation extends Component
{
    ...
    {
        return __('erp.nav.ap_reconciliation');
    }
    ...
    protected function excelSheets(): array
    {
        ...
        $result = app(ApApplicationService::class)->reconcile($company, $book, Carbon::parse($this->asOf));
        $gl = (string) ($result['expected'] ?? '0');
        $subledger = (string) ($result['actual'] ?? '0');

        return [$this->excelKeyValueSheet(
            __('erp.nav.ap_reconciliation'),
            [
                __('erp.reconciliation.gl_balance_ap') => $gl,
                __('erp.reconciliation.subledger_ap') => $subledger,
